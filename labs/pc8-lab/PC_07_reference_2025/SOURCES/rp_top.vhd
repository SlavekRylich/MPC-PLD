...
----------------------------------------------------------------------------------
  ...
  SIGNAL sig_7seg0          : STD_LOGIC_VECTOR (3 DOWNTO 0);
  SIGNAL sig_7seg1          : STD_LOGIC_VECTOR (3 DOWNTO 0);
  SIGNAL sig_7seg2          : STD_LOGIC_VECTOR (3 DOWNTO 0);
  SIGNAL sig_7seg3          : STD_LOGIC_VECTOR (3 DOWNTO 0);
...
----------------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------------------------------------
  -- VIO (Virtual I/O) to drive PWM reference inputs (8 x 8 bits)
  
  vio_drive_pwm_i : VIO_PWM
  PORT MAP (
    clk                 => CLK,
    probe_in0           => pwm_out,
    probe_in1           => cnt_out,
  ...
  ----------------------------------------------------------------------------------------------------------
  -- VIO to monitor PWM output signals (8 x 1 bit) and PWM counter value (CNT_OUT; 8 bits)
  -- VIO to drive 7-segment display using provided driver (4 x 4 bits)
  
  vio_drive_7seg_i : VIO_7SEG
  PORT MAP (
    clk => clk,
    probe_out0 => sig_7seg0,
    probe_out1 => sig_7seg1,
    probe_out2 => sig_7seg2,
    probe_out3 => sig_7seg3
  );
    
  ----------------------------------------------------------------------------------------------------------
  -- ILA (Integrated Logic Analyzer) core to monitor all PWM outputVIO to drive 7-segment display using provided driver (4 x 4 bits)
  -- signals (8 bits) and PWM counter value (CNT_OUT; 8 bits)
  
  ila_monitor_pwm_i : ILA_PWM
  PORT MAP (
      clk => clk,
      probe0 => pwm_out,
      probe1 => cnt_out
  ...
  ila_monitor_sw_butt_i : ILA_PWM
  PORT MAP (
      clk => clk,
      probe0(3 downto 0) => SW,
      probe0(7 downto 4) => "0000",
      probe1(3 downto 0) => BTN,                  
      probe1(7 downto 4) => "0000"
  );
